...
export function parseKernelAddonPathFlag(value) {
    if (typeof value !== "string" || value.length === 0) {
        throw new Error("--fast-grep-kernel-addon must be an explicit absolute .node path when kernel-dev is selected");
    }
...
export function parseKernelHostContractFlag(value) {
    if (value !== KERNEL_HOST_CONTRACT) {
        throw new Error(`--fast-grep-kernel-host-contract must be ${KERNEL_HOST_CONTRACT} when kernel-dev is selected`);
    }
    return value;
}
export function kernelToolMayMutate(toolName) {
...
export class KernelSessionGate {
    ...
    async enterRead() {
        while (this.mutationActive) {
            await new Promise((resolve) => this.readerWaiters.add(resolve));
        }
        this.readers += 1;
        let released = false;
        return () => {
    ...
    async beginMutation(toolCallId) {
        if (this.mutations.has(toolCallId))
            return;
    ...
    finishMutation(toolCallId) {
        if (!this.mutations.delete(toolCallId))
            return false;
...
function kernelHostToolIdentityFailure(pi, expectedGrepParameters) {
    let tools;
    try {
    ...
    for (const name of ["read", "find", "ls"]) {
...
async function stopEngine(active) {
    if (active.kind === "fff") {
        active.value.stop();
        return;
    }
...
export function parseInitialDirtyPathsFlag(value, root) {
    if (value === undefined)
        return undefined;
    ...
    try {
        ...
        throw new Error(`invalid --fast-grep-initial-dirty-paths: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export function registerFastGrepExtension(pi, options) {
    let engine;
    let registeredGrepParameters;
    let pendingKernelMutationReason;
    ...
    pi.registerFlag("fast-grep-kernel-addon", {
        description: "Absolute native .node path required by the non-default kernel-dev backend",
        type: "string",
    });
    pi.registerFlag("fast-grep-kernel-host-contract", {
        description: `Required kernel-dev capability acknowledgement: ${KERNEL_HOST_CONTRACT}`,
        type: "string",
    });
    pi.registerFlag("fast-grep-initial-dirty-paths", {
        description: "Trusted JSON array of initial dirty repository-relative paths; skips only the first git status scan",
        type: "string",
    });
    const createKernelActive = async (root, addonPath, sessionWorktreeSnapshot) => {
        const { KernelMutationFeed: KernelMutationFeedConstructor, OptInKernelEngine: OptInKernelEngineConstructor, } = await import("./kernel-engine.js");
    ...
    const recoverKernel = async (ctx) => {
        if (!kernelNeedsRecovery || kernelGate.mutationActive)
            return;
        if (kernelRecovery !== undefined)
            return kernelRecovery.promise;
        const previous = engine;
        if (previous?.kind !== "kernel" || previous.root !== ctx.cwd)
            return;
        ...
        recovery.promise = (async () => {
            ...
            let candidate;
            try {
                candidate = await createKernelActive(previous.root, previous.addonPath, true);
                ...
                candidate?.value.close();
                console.error(`[pi-fast-grep] kernel recovery failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            finally {
                releaseRead();
            }
        })().finally(() => {
            if (kernelRecovery === recovery)
                kernelRecovery = undefined;
        });
        kernelRecovery = recovery;
        return recovery.promise;
    };
    const ensureEngine = async (ctx, backend) => {
        const kind = backend === "fff"
            ? "fff"
            : backend === "kernel" || backend === "kernel-dev"
                ? "kernel"
                : "fast";
        const kernelAddonPath = backend === "kernel-dev"
        ...
        if (backend === "kernel-dev") {
            parseKernelHostContractFlag(pi.getFlag("fast-grep-kernel-host-contract"));
        }
        ...
        if (backend === "kernel" || backend === "kernel-dev") {
            ...
            const initialInvalidReason = kernelHostToolIdentityFailure(pi, registeredGrepParameters) ?? pendingKernelMutationReason;
            ...
            try {
                ...
                console.error(`[pi-fast-grep] kernel startup failed: ${error instanceof Error ? error.message : String(error)}`);
                active.value.markWorkspaceChanged(active.mutationFeed.reason ?? "kernel_start_failed");
            }
            return active;
        }
        const initialDirtyPaths = parseInitialDirtyPathsFlag(pi.getFlag("fast-grep-initial-dirty-paths"), ctx.cwd);
    ...
    const fallbackDefinitionFor = (root) => {
    ...
    const grep = defineTool({
        ...
        description: "Search code quickly in large repositories with regex and path/glob filters, " +
            ...
            try {
                ...
                try {
                    ...
                    if (activeEngine.kind === "fff") {
                        if (backend !== "fff")
                            throw new Error("FFF engine selected for a non-FFF request");
                    ...
                    const fallback = await fallbackDefinitionFor(ctx.cwd).execute(toolCallId, params, signal, onUpdate, ctx);
    ...
    const noteKernelMutation = (reason) => {
        if (engine?.kind !== "kernel")
            return;
        kernelEpoch += 1;
    ...
    pi.on("tool_execution_start", (event) => {
        if (!kernelToolMayMutate(event.toolName))
            return;
    ...
    pi.on("tool_result", (event, ctx) => {
        ...
        if (event.toolName === "write" || event.toolName === "edit") {
            if (engine.kind === "fff")
                ...
                engine.value.markToolPath(event.input.path, ctx.cwd);
            return;
        }
...
export default function fastGrepExtension(pi) {
    registerFastGrepExtension(pi, {
        defaultBackend: "auto",
        moduleDirectory: import.meta.dirname,
    });
}
export function packagedFastGrepExtension(pi) {
    registerFastGrepExtension(pi, {
