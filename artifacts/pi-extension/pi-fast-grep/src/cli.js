...
function parseArgs(argv) {
    ...
    if (first === "index" || first === "search")
        args.shift();
    let root = process.cwd();
    let backend = "auto";
    let json = false;
    const request = { pattern: "", context: 2, limit: 100, hidden: true };
    const positional = [];
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--root" || arg === "--path" || arg === "--glob" || arg === "--backend" || arg === "--limit" || arg === "--context" || arg === "-C") {
            ...
            if (arg === "--root")
                ...
                if (value !== "auto" && value !== "instant" && value !== "normal")
                ...
                request.context = Number(value);
            continue;
        }
        if (arg === "-i" || arg === "--ignore-case")
            request.ignoreCase = true;
        else if (arg === "-F" || arg === "--literal")
            request.literal = true;
        else if (arg === "-U" || arg === "--multiline")
    ...
    if (command === "search") {
        if (positional.length !== 1)
            throw new Error("search requires exactly one PATTERN");
...
async function main() {
    ...
    try {
        ...
        if (options.json)
            ...
            process.stdout.write(`${formatSearchResult(result, options.request).text}\n`);
    }
    finally {
        await engine.stop();
    }
