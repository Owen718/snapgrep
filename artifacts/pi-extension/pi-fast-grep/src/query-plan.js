 ...
 */
function extractPureRegexLiteral(pattern) {
    if (pattern.includes("\n") || pattern.includes("\r")) {
...
function escapeZoektRegexLiteral(literal) {
    return literal.replace(/[\\.^$|?*+()[\]{}]/gu, "\\$&");
}
/** Quote a Zoekt query-language atom (a separate layer from JSON encoding). */
function quoteZoektAtom(value) {
    return `"${value.replace(/\\/gu, "\\\\").replace(/"/gu, '\\"')}"`;
}
export function buildZoektLiteralQuery(literal, ignoreCase = false) {
...
function buildZoektExactMatchQuery(literal) {
 ...
 */
function pathFileFilter(relativeSearchRoot) {
    if (relativeSearchRoot === undefined || relativeSearchRoot === "" || relativeSearchRoot === ".") {
        return undefined;
    }
    if (relativeSearchRoot.includes("\0")
...
function scopedClause(kind, regex, ignoreCase) {
 ...
 */
export function planZoektQuery(request, context = {}) {
    ...
    const extraction = request.literal === true
    ...
    const query = fileFilters.length === 0
        ...
        : [
            "type:file",
            scopedClause("content", escapeZoektRegexLiteral(extraction.literal), request.ignoreCase === true),
    ...
    const exactMatchQuery = (request.ignoreCase !== true
