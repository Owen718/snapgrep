import type { SearchMatch, SearchMetadata, SearchRequest, SearchResult } from "./types.js";

const MAX_OUTPUT_BYTES = 50 * 1024;
const MAX_LINE_CHARS = 500;

export interface FastGrepDetails {
  matches: SearchMatch[];
...
function truncateLine(value: string): { text: string; truncated: boolean } {
  if (value.length <= MAX_LINE_CHARS) return { text: value, truncated: false };
  return { text: `${value.slice(0, MAX_LINE_CHARS)}... [truncated]`, truncated: true };
}

function collectDisplayLines(matches: readonly SearchMatch[]): Map<string, Map<number, DisplayLine>> {
  const files = new Map<string, Map<number, DisplayLine>>();
...
function backendSummary(metadata: SearchMetadata): string {
  ...
  if (metadata.fallbackReason) pieces.push(`fallback=${metadata.fallbackReason}`);
  return `[fast_grep: ${pieces.join(", ")}]`;
}

export function formatSearchResult(result: SearchResult, request: SearchRequest): FormattedSearchResult {
  const output: string[] = [];
  ...
  for (const [filePath, lines] of [...display].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    for (const line of [...lines.values()].sort((a, b) => a.lineNumber - b.lineNumber)) {
      if (previousLine !== undefined && line.lineNumber > previousLine + 1) output.push("--");
  ...
  if (output.length === 0) {
    ...
    if (result.metadata.actualBackend === "zoekt") {
      output.push(
        "If this conflicts with known repository evidence, verify once with `rg --no-config` in bash.",
      );
    }
  }
  ...
  if (result.metadata.truncated) {
    ...
    notices.push(
      `Showing ${result.metadata.displayedMatches} of ${total} matches (limit ${limit}). Increase limit or narrow path/glob`,
    );
  }
  if (linesTruncated) notices.push(`Some lines were truncated to ${MAX_LINE_CHARS} characters; use read for full text`);
  if (notices.length > 0) output.push("", `[${notices.join(". ")}]`);
  ...
  if (Buffer.byteLength(text) > MAX_OUTPUT_BYTES) {
    ...
    for (const line of text.split("\n")) {
      ...
      if (bytes + lineBytes > MAX_OUTPUT_BYTES - 128) break;
      kept.push(line);
      bytes += lineBytes;
    }
  ...
  if (outputTruncated) details.outputTruncated = true;
  return { text, details };
}
