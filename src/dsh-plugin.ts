...
export interface Config {
  /** Max matches returned inline by one call. */
  maxMatches?: number;
  /** Max paths returned inline by one glob call. */
  maxPaths?: number;
}
 ...
 */
function formatMatches(matches: readonly GrepOutputMatch[]): string {
  if (matches.length === 0) return "No matches found.";
  const byFile = new Map<string, GrepOutputMatch[]>();
 ...
 */
class EngineCache {
  private readonly engines = new Map<string, Promise<OptInKernelEngine | undefined>>();
  private readonly feeds = new Map<string, KernelMutationFeed>();

  constructor(private readonly moduleDirectory: string) {}

  get(root: string): Promise<OptInKernelEngine | undefined> {
  ...
  invalidate(reason: string): void {
    for (const feed of this.feeds.values()) feed.mark(reason);
    this.closeAll();
  }

  private async start(root: string): Promise<OptInKernelEngine | undefined> {
    let engine: OptInKernelEngine | undefined;
    try {
...
export function apply(ctx: any, config: Config = {}): void {
  ...
  const tool = defineTool({
    name: "grep",
    description:
      "Search file contents with a ripgrep regular expression. Returns matching "
    ...
    parameters: {
      ...
      path: {
        type: "string",
        description:
          "File or directory to search. Defaults to the session workspace; a "
          + "relative path resolves against it.",
      },
      include: {
        type: "string",
        description:
          "One glob filter for which files to search (e.g. \"*.ts\"). Not a "
    ...
    output: {
      ...
      render: (_args: GrepArgs, value: { matches: GrepOutputMatch[] }) => [
        { type: "text" as const, text: formatMatches(value.matches) },
      ],
    },
    async execute(args: GrepArgs, exec: ExecutionContext) {
  ...
  const globTool = defineTool({
    name: "glob",
    description:
      "Find files by path pattern. A pattern with no \"/\" matches basenames at "
    ...
    output: {
      ...
      render: (_args: GlobArgs, value: { paths: string[] }) => [
        {
          type: "text" as const,
    ...
    async execute(args: GlobArgs, exec: ExecutionContext) {
      const workdir = exec.agent?.session?.header?.cwd ?? process.cwd();
      const argv = [
        "--files",
        `--glob=${args.pattern}`,
  ...
  ctx.on?.("tools/pre-execute", async (exec: { toolName?: string }, next: () => Promise<unknown>) => {
