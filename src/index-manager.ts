...
function ownedTemporaryName(name: string): boolean {
...
export async function vacuumStaleIndexTemporaries(
  indexDir: string,
  ...
  for (const entry of entries) {
    if (!entry.isFile() || !ownedTemporaryName(entry.name)) continue;
    ...
    if (nowMs - info.mtimeMs < staleTempMinAgeMs) continue;
...
export async function vacuumOwnedIndexArtifacts(
  ...
  for (const entry of entries) {
    ...
    if (!ownedSessionArtifact) continue;
