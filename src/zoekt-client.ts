...
import type { EligibleZoektQueryPlan } from "./query-plan.js";

const DEFAULT_TIMEOUT_MS = 5_000;
const DEFAULT_MAX_FILES = 10_000;
const MAX_EXACT_MATCH_LINES = 1_000_000;

export interface ZoektClientOptions {
  baseUrl: string;
  timeoutMs?: number;
  maxFiles?: number;
...
export interface ZoektRequestOptions {
  signal?: AbortSignal;
}

export class ZoektClientError extends Error {
  override readonly name: string = "ZoektClientError";
...
export class ZoektHttpError extends ZoektClientError {
  override readonly name = "ZoektHttpError";
...
export class ZoektApiError extends ZoektClientError {
  override readonly name = "ZoektApiError";
}

export class ZoektResponseError extends ZoektClientError {
  override readonly name = "ZoektResponseError";
}

export class ZoektTimeoutError extends ZoektClientError {
  override readonly name = "ZoektTimeoutError";
...
export class ZoektNetworkError extends ZoektClientError {
  override readonly name = "ZoektNetworkError";
}
...
type HealthInput = ZoektRequestOptions | AbortSignal | undefined;
type SearchInput = string | Pick<
  EligibleZoektQueryPlan,
  "query" | "mandatoryLiteral" | "exactMatchQuery"
>;
...
function abortSignalFrom(input: HealthInput): AbortSignal | undefined {
  if (input === undefined) {
    return undefined;
  }
  if (typeof (input as AbortSignal).aborted === "boolean") {
    return input as AbortSignal;
  }
  return (input as ZoektRequestOptions).signal;
}

function signalIsAborted(signal: AbortSignal | undefined): signal is AbortSignal {
...
function parseExactMatches(
  result: JsonObject,
  files: readonly JsonObject[],
  literal: string,
): {
  matches: NonNullable<CandidateResult["exactMatches"]>;
  carriageReturnPaths: string[];
} | undefined {
  if (
    nonNegativeSafeInteger(result.Crashes) !== 0
  ...
  const matches: NonNullable<CandidateResult["exactMatches"]> = [];
  const carriageReturnPaths = new Set<string>();
  const seenFiles = new Set<string>();
  let parsedLineMatches = 0;

  for (const file of files) {
    const fileName = normalizeFileName(file.FileName);
    if (seenFiles.has(fileName)) return undefined;
    seenFiles.add(fileName);
    if (
      file.ChunkMatches !== undefined
    ...
    const lineMatches = (file.LineMatches ?? []) as unknown[];
    for (const rawLineMatch of lineMatches) {
      parsedLineMatches += 1;
      if (!isObject(rawLineMatch) || rawLineMatch.FileName !== false) return undefined;
      ...
      if (firstLf !== -1 && firstLf !== line.length - 1) return undefined;
      ...
      for (const rawFragment of rawLineMatch.LineFragments) {
        if (!isObject(rawFragment)) return undefined;
...
function baseVersion(files: readonly JsonObject[]): {
  version?: string;
  state: CandidateResult["baseVersionState"];
} {
  const baseFiles = files.filter(
    (file) => typeof file.Repository !== "string" || !file.Repository.startsWith("fast-grep-overlay/"),
  );
  if (baseFiles.length === 0) return { state: "no_base_files" };
...
export class ZoektClient {
  ...
  constructor(options: ZoektClientOptions | string) {
    const normalizedOptions: ZoektClientOptions = typeof options === "string"
      ? { baseUrl: options }
      : options;
    ...
    this.timeoutMs = requirePositiveInteger(
      normalizedOptions.timeoutMs ?? DEFAULT_TIMEOUT_MS,
      "timeoutMs",
    );
    this.maxFiles = requirePositiveInteger(
      normalizedOptions.maxFiles ?? DEFAULT_MAX_FILES,
      "maxFiles",
    );
  ...
  async health(options?: ZoektRequestOptions | AbortSignal): Promise<boolean> {
    const { payload } = await this.requestJson(
      "/healthz",
      { method: "GET" },
      abortSignalFrom(options),
    );
  ...
  async healthCheck(options?: ZoektRequestOptions | AbortSignal): Promise<boolean> {
    return this.health(options);
  }

  async isHealthy(options?: ZoektRequestOptions | AbortSignal): Promise<boolean> {
    return this.health(options);
  }

  async search(input: SearchInput, options: ZoektSearchOptions | number = {}): Promise<CandidateResult> {
    ...
    const normalizedOptions: ZoektSearchOptions = typeof options === "number"
      ? { maxFiles: options }
      : options;
    ...
    const { payload, jsonDecodeMs } = await this.requestJson(
      "/api/search",
      {
        method: "POST",
    ...
    const rawFiles = (result.Files ?? []) as unknown[];
    const fileObjects: JsonObject[] = rawFiles.map((file) => {
      if (!isObject(file)) {
    ...
    const transportSerializationMs = Math.max(
      0,
      roundTripMs - (serverDurationMs ?? 0) - jsonDecodeMs,
    );
  ...
  async searchCandidates(
    input: SearchInput,
    options: ZoektSearchOptions | number = {},
