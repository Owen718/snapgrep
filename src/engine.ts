...
import { IndexManager, type IndexManagerOptions } from "./index-manager.js";
...
interface CandidateExpansionOptions {
  initialMaxFiles: number;
  maxCandidateFilesConfigured: boolean;
  signal?: AbortSignal;
}
 ...
 */
export async function searchCandidatesWithBoundedExpansion(
  search: (maxFiles: number, signal: AbortSignal | undefined) => Promise<CandidateResult>,
  ...
  const {
    ...
    roundTripMs: expandedRoundTripMs,
    serverDurationMs: expandedServerDurationMs,
    jsonDecodeMs: expandedJsonDecodeMs,
    transportSerializationMs: expandedTransportSerializationMs,
    ...expandedResult
  } = expanded;
  ...
  return {
    ...
    transportSerializationMs:
      initial.transportSerializationMs + expandedTransportSerializationMs,
...
export interface FastGrepEngineOptions {
  root: string;
  indexManager?: IndexManager;
  dirtyTracker?: DirtyTracker;
  initialDirtyPaths?: readonly string[];
  requestedBackend?: "auto" | "instant" | "normal";
  zoektTimeoutMs?: number;
  maxCandidateFiles?: number;
  indexOptions?: Omit<IndexManagerOptions, "root">;
}
...
function mergeMatches(groups: readonly (readonly SearchMatch[])[]): SearchMatch[] {
  ...
  await mapLimited(
    ...
    async (relativePath) => {
      ...
      try {
        ...
        if (info.isFile()) result.push(relativePath);
      } catch {
...
export class FastGrepEngine {
  ...
  private readonly requestedBackend: "auto" | "instant" | "normal";
  private readonly zoektTimeoutMs: number;
  private readonly maxCandidateFiles: number | undefined;
  private readonly eligibleFilesCache = new Map<string, { generation: number; paths: ReadonlySet<string> }>();
  private unindexedFilesCache: {
    key: string;
    excludedPaths: readonly string[];
    paths: readonly string[];
    binaryPaths: readonly string[];
    lateBinaryOffsets: ReadonlyMap<string, number>;
    binaryFiles: number;
  ...
  private filteredFilesCache: { commit: string; paths: readonly string[] } | undefined;
  private indexedBinaryCache: {
    key: string;
    classified: Set<string>;
    binary: Set<string>;
    carriageReturn: Set<string>;
    firstNulOffsets: Map<string, number>;
  } | undefined;
  private acknowledgedCommit: string | undefined;
  private dirtyUnsubscribe: (() => void) | undefined;
  private overlayTimer: NodeJS.Timeout | undefined;
  private overlayPromise: Promise<void> | undefined;
  private overlayRequested = false;
  private overlayLastError: Error | undefined;
  ...
  constructor(options: FastGrepEngineOptions) {
  ...
  async start(options: StartOptions = {}): Promise<void> {
      ...
      if (this.startPromise === undefined) {
        this.startPromise = (async () => {
          ...
          this.dirtyUnsubscribe = this.dirtyTracker.onDirty((_generation, headMayHaveChanged) => {
            if (headMayHaveChanged) this.indexManager.invalidateCurrentCommit();
    ...
    if (!this.started || this.stopping || epoch !== this.lifecycleEpoch) return;
  ...
  async flushIncrementalIndex(timeoutMs = 5_000): Promise<void> {
    ...
    try {
      await Promise.race([
        ...
        new Promise<never>((_resolve, reject) => {
          timeout = setTimeout(
            () => reject(new Error(`incremental index did not become ready within ${timeoutMs} ms`)),
            timeoutMs,
          );
          timeout.unref();
        }),
      ]);
      if (this.overlayLastError !== undefined) throw this.overlayLastError;
    } finally {
      if (timeout !== undefined) clearTimeout(timeout);
    }
  }

  private scheduleIncrementalIndex(delayMs = 100): void {
    if (!this.started || this.stopping) return;
    this.overlayRequested = true;
    if (this.overlayPromise !== undefined) return;
    if (this.overlayTimer !== undefined) clearTimeout(this.overlayTimer);
  ...
  private runIncrementalIndex(): Promise<void> {
    if (!this.started || this.stopping) return Promise.resolve();
    if (this.overlayPromise !== undefined) return this.overlayPromise;
  ...
  private async incrementalIndexLoop(): Promise<void> {
    while (this.overlayRequested && this.started && !this.stopping) {
      ...
      try {
        ...
        if (!this.indexManager.ready || status.mode !== "git") return;
        ...
        if (latest.paths.size > 0) this.overlayRequested = true;
      } catch (error) {
  ...
  async search(
    ...
    options: { signal?: AbortSignal; backend?: "auto" | "instant" | "normal" } = {},
    ...
    if (!this.started) await this.start({ waitForIndex: false });
    let dirty;
    try {
      await this.dirtyTracker.refreshIfNeeded();
      dirty = await this.dirtyTracker.snapshot();
    } catch (error) {
      if (!(error instanceof DirtyCoverageError)) throw error;
    ...
    if (dirty.paths.size > 0) this.scheduleIncrementalIndex();
    const searchRoot = path.resolve(this.root, request.path ?? ".");
    if (!isWithin(this.root, searchRoot)) {
      return this.fallback(
        request,
        "search path is outside the indexed repository root",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (!this.indexManager.ready || !this.indexManager.url) {
      ...
      return this.fallback(
        request,
        `index unavailable (${status.lifecycle})`,
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (status.mode !== "git") {
      return this.fallback(
        request,
        "non-Git workspace index is not trusted across sessions; using recall-safe ripgrep",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (status.submodulesPresent) {
      return this.fallback(
        request,
        "repository has submodules that are not covered by the base Zoekt shard",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (!commitBeforeQuery.known || currentCommit === undefined) {
      ...
      return this.fallback(
        request,
        "repository HEAD freshness is pending",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    try {
      ...
      [candidates, unindexed] = await Promise.all([
        ...
        this.unindexedFiles(
          this.indexManager.url,
          status.indexedCommit,
          status.overlayRevision ?? 0,
          options.signal,
        ),
      ]);
    } catch (error) {
      ...
      return this.fallback(
        request,
        `sidecar query or index-coverage check failed: ${errorMessage(error)}`,
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (candidates.baseVersionState === "missing" || candidates.baseVersionState === "mixed") {
      return this.fallback(
        request,
        `sidecar base-shard version is ${candidates.baseVersionState}`,
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (
      candidates.indexedCommit !== undefined &&
      status.indexedCommit !== undefined &&
      candidates.indexedCommit !== status.indexedCommit
    ) {
      return this.fallback(
        request,
        "sidecar served a different indexed commit",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (
      ...
      return this.fallback(
        request,
        "repository commit changed during the indexed query",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    if (candidates.truncated) {
      return this.fallback(
        request,
        "candidate set was truncated, so exact ordering and totals require ripgrep",
        startedAt,
        dirty.paths.size,
        options.signal,
        requestedBackend,
    ...
    const realtimeCandidates = await existingPaths(
      this.root,
      new Set([...dirtyPaths, ...indexedOverlayPaths]),
      options.signal,
    );
    const verificationRequest: SearchRequest = { ...request, limit: null };
    ...
    try {
        ...
        ? [
            {
              ...
              carriageReturnPaths: [] as string[],
              lateBinaryOffsets: new Map<string, number>(),
              binaryFiles: 0,
            },
            {
      ...
      const eligiblePaths = await this.eligibleFiles(
        request,
        dirty.generation,
        status.indexedCommit,
        status.overlayRevision ?? 0,
      ...
      const lateBinaryOffsets = new Map<string, number>();
      for (const source of [
        ...
        for (const [relativePath, offset] of source) {
          ...
          if (previous === undefined || offset < previous) lateBinaryOffsets.set(relativePath, offset);
        }
      }
      ...
      if (candidates.exactMatches !== undefined && !explicitFileSearch) {
        ...
        const indexedCarriageReturnPaths = indexedClassified.paths.filter((relativePath) =>
          carriageReturnPaths.has(relativePath));
        ...
        const directIndexedMatches = candidates.exactMatches
          .filter((match) =>
            eligiblePaths.has(match.path) && !directExcludedPaths.has(match.path))
          .map((match) => ({
            ...match,
      ...
      if (latestDirty.generation !== dirty.generation) {
        return this.fallback(
          request,
          "workspace changed during the indexed query",
          startedAt,
          latestDirty.paths.size,
          options.signal,
          requestedBackend,
        );
      }
      ...
      return this.fallback(
        request,
        error instanceof DirtyCoverageError
          ? `workspace dirty coverage unavailable: ${error.message}`
  ...
  private candidateLimit(request: SearchRequest): number {
    if (this.maxCandidateFiles !== undefined) return this.maxCandidateFiles;
    if (request.limit === null) return 1_000_000;
    return Math.max(10_000, (request.limit ?? 100) * 20);
  }

  private async unindexedFiles(
    baseUrl: string,
    ...
    const [result, binaryResult, filteredPaths] = await Promise.all([
      ...
      client.search(
        'type:file case:yes content:"NOT-INDEXED: contains binary content"',
        searchOptions,
      ),
    ...
    if (
      result.baseVersionState === "missing"
    ...
    const excludedPaths = await existingPaths(
      this.root,
      new Set([...result.files, ...filteredPaths]),
      signal,
    );
    const binary = new Set(binaryResult.files);
    const firstNulOffsets = new Map<string, number>();
    ...
    const lateBinaryOffsets = new Map(
      binaryPaths
        .map((relativePath) => [relativePath, firstNulOffsets.get(relativePath)] as const)
        .filter((entry): entry is readonly [string, number] =>
          entry[1] !== undefined && entry[1] >= RIPGREP_INITIAL_BINARY_SCAN_BYTES),
    );
    const cached = {
      key,
      excludedPaths,
      ...
      transportSerializationMs: Math.max(
        result.transportSerializationMs,
        binaryResult.transportSerializationMs,
      ),
  ...
  private async excludeBinaryPaths(
    ...
    const cache = persist
      ...
      : {
          ...
          carriageReturn: new Set<string>(),
          firstNulOffsets: new Map<string, number>(),
        };
    ...
    await mapLimited(unknown, 16, async (relativePath) => {
      ...
      if (markers.containsCarriageReturn) cache.carriageReturn.add(relativePath);
    ...
    const carriageReturnPaths = uniquePaths.filter(
      (relativePath) => cache.carriageReturn.has(relativePath),
    );
    const lateBinaryOffsets = new Map(
      binaryPaths
        .map((relativePath) => [relativePath, cache.firstNulOffsets.get(relativePath)] as const)
        .filter((entry): entry is readonly [string, number] =>
          entry[1] !== undefined && entry[1] >= RIPGREP_INITIAL_BINARY_SCAN_BYTES),
    );
    return {
  ...
  private binaryCache(key: string): NonNullable<FastGrepEngine["indexedBinaryCache"]> {
  ...
  private async filteredWorktreePaths(indexedCommit: string | undefined): Promise<readonly string[]> {
    const commit = indexedCommit ?? "workspace";
    if (this.filteredFilesCache?.commit === commit) return this.filteredFilesCache.paths;
    let paths: string[] = [];
    try {
  ...
  private async eligibleFiles(
    ...
    if (cached?.generation === generation) return cached.paths;
    ...
    if (latestGeneration === generation) {
      if (this.eligibleFilesCache.size >= 64) {
        const oldest = this.eligibleFilesCache.keys().next().value as string | undefined;
        if (oldest !== undefined) this.eligibleFilesCache.delete(oldest);
      }
  ...
  private async fallback(
    ...
    signal: AbortSignal | undefined,
    requestedBackend: "auto" | "instant" | "normal",
    commits: { indexedCommit?: string; currentCommit?: string } = {},
  ): Promise<SearchResult> {
    ...
    if (commits.indexedCommit !== undefined) result.metadata.indexedCommit = commits.indexedCommit;
    if (commits.currentCommit !== undefined) result.metadata.currentCommit = commits.currentCommit;
    return result;
  }
}

async function requestTargetsRegularFile(root: string, request: SearchRequest): Promise<boolean> {
  if (request.path === undefined) return false;
  return (await stat(path.resolve(root, request.path))).isFile();
}

async function firstNulOffset(
  absolutePath: string,
  signal?: AbortSignal,
...
async function scanFileMarkers(
  absolutePath: string,
  signal?: AbortSignal,
): Promise<{ firstNulOffset?: number; containsCarriageReturn: boolean }> {
  ...
  try {
    ...
    while (true) {
      ...
      if (bytesRead === 0) return { containsCarriageReturn };
      const chunk = buffer.subarray(0, bytesRead);
      if (chunk.includes(13)) containsCarriageReturn = true;
