...
export type TraceRunStatus = "completed" | "aborted";
export type TraceEventStatus = "ok" | "error" | "aborted";
export type TraceOperation = "thinking" | "grep" | "read" | "edit" | "write" | "bash" | "find" | "ls" | "tool";

export interface TraceToolInputSignature {
  schemaVersion: 1;
  /** Sorted argument names are safe structural evidence; argument values are never persisted. */
  argumentKeys: string[];
  /** Digest of a canonical, cwd-normalized representation of all arguments. */
  digest: string;
  patternHash?: string;
  pathHash?: string;
  globHash?: string;
...
export interface TraceToolEvent extends TraceEventBase {
  op: Exclude<TraceOperation, "thinking">;
  tool: string;
  toolCallId: string;
  actualSearchBackend?: string;
  fallbackReason?: string;
  resultCount?: number;
  truncated?: boolean;
  inputSignature?: TraceToolInputSignature;
}

export type TraceEvent = TraceThinkingEvent | TraceToolEvent;
...
export interface TraceRun extends TraceRunMetadata {
...
const RUN_STATUSES = new Set<TraceRunStatus>(["completed", "aborted"]);
const EVENT_STATUSES = new Set<TraceEventStatus>(["ok", "error", "aborted"]);
...
function normalizeToolOperation(tool: string): TraceToolEvent["op"] {
  switch (tool) {
    case "grep":
    case "fast_grep":
      return "grep";
...
function readFastGrepMetadata(tool: string, result: unknown): FastGrepMetadata {
  ...
  if ((tool !== "grep" && tool !== "fast_grep") || !isRecord(result)) return {};
  ...
  const countValue =
    metadata.resultCount ?? metadata.displayedMatches ?? metadata.totalMatches ??
    (details && Array.isArray(details.matches) ? details.matches.length : undefined);
  const truncatedValue = metadata.truncated;
  const extracted: FastGrepMetadata = {};
  if (typeof actualValue === "string" && actualValue.length > 0) extracted.actualSearchBackend = actualValue;
  if (typeof fallbackValue === "string" && fallbackValue.length > 0) extracted.fallbackReason = fallbackValue;
  if (typeof countValue === "number" && Number.isFinite(countValue) && countValue >= 0) {
    extracted.resultCount = countValue;
  }
  if (typeof truncatedValue === "boolean") extracted.truncated = truncatedValue;
  return extracted;
}
...
function pathForSignature(value: string, cwd: string | undefined): string {
  if (!cwd) return path.normalize(value);
...
function canonicalInput(value: unknown, cwd: string | undefined, key?: string): unknown {
  if (value === undefined) return { $undefined: true };
  if (value === null || typeof value === "boolean" || typeof value === "number") return value;
  if (typeof value === "string") {
    return key === "path" || key === "file_path" ? pathForSignature(value, cwd) : value;
  }
  if (Array.isArray(value)) return value.map((item) => canonicalInput(item, cwd));
...
function stableInput(value: unknown, cwd?: string): string {
 ...
 */
export function toolInputSignature(tool: string, input: unknown, cwd?: string): TraceToolInputSignature {
  const record = isRecord(input) ? input : {};
  const signature: TraceToolInputSignature = {
    schemaVersion: 1,
    argumentKeys: Object.keys(record).sort(),
    digest: sha256(`${tool}\0${stableInput(input, cwd)}`),
  };
  if (typeof record.pattern === "string") signature.patternHash = sha256(record.pattern);
  const pathValue = typeof record.path === "string"
    ? record.path
    : typeof record.file_path === "string" ? record.file_path : undefined;
  if (pathValue !== undefined) signature.pathHash = sha256(pathForSignature(pathValue, cwd));
  if (typeof record.glob === "string") signature.globHash = sha256(record.glob);
...
function tokenNumber(record: Record<string, unknown>, key: string): number {
  const value = record[key];
  return typeof value === "number" && Number.isFinite(value) && value >= 0 ? value : 0;
}

function extractTokenUsage(value: unknown): TraceTokenUsage | undefined {
  ...
  const extracted: TraceTokenUsage = {
    input,
    output,
    cacheRead,
    cacheWrite,
  ...
  if (typeof usage.reasoning === "number") extracted.reasoning = reasoning;
  if (isRecord(usage.cost) && typeof usage.cost.total === "number") extracted.costTotal = cost;
  return extracted;
}
 ...
 */
export function computeThinkingSpans(
  tools: readonly Pick<TraceToolEvent, "startMs" | "endMs">[],
  totalMs: number,
): TraceThinkingEvent[] {
  ...
  const intervals = tools
    .map(({ startMs, endMs }) => {
      ...
      if (endMs < startMs) throw new Error("tool endMs must not precede startMs");
      if (endMs > totalMs && !sameTime(endMs, totalMs)) throw new Error("tool endMs exceeds totalMs");
  ...
  const thinking: TraceThinkingEvent[] = [];
  let cursor = 0;
  const addSpan = (startMs: number, endMs: number) => {
    if (endMs <= startMs || sameTime(startMs, endMs)) return;
    thinking.push({
      op: "thinking",
      startMs,
      endMs,
...
export function buildTraceEvents(tools: readonly TraceToolEvent[], totalMs: number): TraceEvent[] {
  const events: TraceEvent[] = [...tools, ...computeThinkingSpans(tools, totalMs)];
  return events.sort((left, right) => {
    const byStart = left.startMs - right.startMs;
    if (byStart !== 0) return byStart;
    if (left.op === "thinking" && right.op !== "thinking") return 1;
    if (right.op === "thinking" && left.op !== "thinking") return -1;
    return left.endMs - right.endMs;
  });
}

function validateDigest(value: unknown, label: string): void {
  if (typeof value !== "string" || !/^[a-f0-9]{64}$/u.test(value)) {
    throw new Error(`${label} must be a lowercase SHA-256 digest`);
  }
}

function validateInputSignature(value: unknown, label: string): void {
  ...
  const argumentKeys = value.argumentKeys as string[];
  ...
  for (const key of ["patternHash", "pathHash", "globHash", "payloadHash"] as const) {
    if (value[key] !== undefined) validateDigest(value[key], `${label}.${key}`);
  }
}

function validateTokenUsage(value: unknown): void {
  if (!isRecord(value)) throw new Error("tokenUsage must be an object");
  for (const key of ["input", "output", "cacheRead", "cacheWrite", "totalTokens"] as const) {
    if (typeof value[key] !== "number") throw new Error(`tokenUsage.${key} must be a number`);
...
export function validateTrace(value: unknown): asserts value is TraceRun {
  ...
  if (value.schemaVersion !== TRACE_SCHEMA_VERSION) throw new Error("unsupported trace schemaVersion");
  for (const key of [
    "runId",
    "workflowId",
    "comparisonId",
  ...
  if (!/^[a-f0-9]{64}$/u.test(promptHash)) throw new Error("promptHash must be a lowercase SHA-256 digest");
  ...
  if (typeof value.totalMs !== "number") throw new Error("totalMs must be a number");
  assertFiniteTime(value.totalMs, "totalMs");
  if (typeof value.status !== "string" || !RUN_STATUSES.has(value.status as TraceRunStatus)) {
    throw new Error("invalid trace status");
  }
  if (!Array.isArray(value.events)) throw new Error("events must be an array");
  if (value.tokenUsage !== undefined) validateTokenUsage(value.tokenUsage);

  const toolEvents: TraceToolEvent[] = [];
  const thinkingEvents: TraceThinkingEvent[] = [];
  const toolCallIds = new Set<string>();
  for (const [index, candidate] of value.events.entries()) {
    if (!isRecord(candidate)) throw new Error(`events[${index}] must be an object`);
    if (typeof candidate.op !== "string" || !OPERATIONS.has(candidate.op as TraceOperation)) {
    ...
    if (candidate.endMs < candidate.startMs) throw new Error(`events[${index}] ends before it starts`);
    ...
    if (typeof candidate.status !== "string" || !EVENT_STATUSES.has(candidate.status as TraceEventStatus)) {
      throw new Error(`events[${index}] has an invalid status`);
    }

    if (candidate.op === "thinking") {
      if (candidate.status !== "ok") throw new Error(`events[${index}] thinking status must be ok`);
      thinkingEvents.push(candidate as unknown as TraceThinkingEvent);
      continue;
    }

    const tool = nonEmptyString(candidate, "tool");
    const toolCallId = nonEmptyString(candidate, "toolCallId");
    if (toolCallIds.has(toolCallId)) throw new Error(`duplicate toolCallId ${toolCallId}`);
    ...
    toolEvents.push(candidate as unknown as TraceToolEvent);
  }
...
export class TraceRecorder {
  private readonly metadata: TraceRunMetadata;
  ...
  private readonly tools: TraceToolEvent[] = [];
  private origin: number | undefined;
  private lastObserved: number | undefined;
  private startedAt: string | undefined;
  private finalized: TraceRun | undefined;
  private tokenUsage: TraceTokenUsage | undefined;

  constructor(metadata: TraceRunMetadata, options: TraceRecorderOptions = {}) {
  ...
  begin(at?: number): void {
    if (this.finalized) throw new Error("trace is already finalized");
  ...
  startTool(toolCallId: string, tool: string, at?: number, input?: unknown, cwd?: string): void {
    this.assertOpen();
    if (!toolCallId || !tool) throw new Error("toolCallId and tool must be non-empty strings");
    ...
    const active: ActiveTool = { tool, startMs: elapsed };
    if (input !== undefined) active.inputSignature = toolInputSignature(tool, input, cwd);
    this.activeTools.set(toolCallId, active);
  }

  endTool(toolCallId: string, result: unknown, isError: boolean, at?: number, expectedTool?: string): void {
    this.assertOpen();
    const active = this.activeTools.get(toolCallId);
    if (!active) throw new Error(`tool_execution_end without matching start for ${toolCallId}`);
    ...
    if (endMs < active.startMs) throw new Error(`tool_execution_end precedes start for ${toolCallId}`);
  ...
  toolStart(toolCallId: string, tool: string, at?: number): void {
    this.startTool(toolCallId, tool, at);
  }

  toolEnd(toolCallId: string, result: unknown, isError: boolean, at?: number, expectedTool?: string): void {
  ...
  addTokenUsage(messageOrUsage: unknown): void {
    this.assertOpen();
  ...
  finish(status: TraceRunStatus = "completed", at?: number): TraceRun {
    if (this.finalized) return this.finalized;
    if (!RUN_STATUSES.has(status)) throw new Error(`invalid trace status: ${status}`);
    if (this.origin === undefined) this.begin(at);
  ...
  private assertOpen(): void {
    if (this.finalized) throw new Error("trace is already finalized");
    if (this.origin === undefined) throw new Error("trace has not started");
  }

  private observe(at: number | undefined, label: string): number {
    if (this.origin === undefined) throw new Error("trace has not started");
    const observed = at ?? this.now();
...
function validateMetadata(metadata: TraceRunMetadata): void {
  const candidate: Record<string, unknown> = { ...metadata };
  for (const key of [
    "runId",
    "workflowId",
    "comparisonId",
  ...
  if (!Number.isInteger(metadata.attempt) || metadata.attempt < 1) throw new Error("attempt must be a positive integer");
...
export async function writeTraceAtomic(outputPath: string, trace: TraceRun): Promise<void> {
  validateTrace(trace);
